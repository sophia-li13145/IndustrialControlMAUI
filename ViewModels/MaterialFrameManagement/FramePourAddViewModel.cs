...
public partial class FramePourAddViewModel : ObservableObject
{
    ...
    [RelayCommand] private void PickSource(FramePourAddSourceFrameItem? i) { if (i is null) return; SelectedSource = i; foreach (var x in SourceFrameList) x.IsSelected = ReferenceEquals(x, i); IsSourcePickerVisible = false; OnPropertyChanged(nameof(HasSelectedSource)); Refresh(); }
    ...
    [RelayCommand] private void PickTarget(FramePourAddTargetFrameItem? i) { if (i is null) return; SelectedTarget = i; foreach (var x in TargetFrameList) x.IsSelected = ReferenceEquals(x, i); IsTargetPickerVisible = false; OnPropertyChanged(nameof(HasSelectedTarget)); Refresh(); }
