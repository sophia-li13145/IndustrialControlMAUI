...
public partial class FrameMergeAddViewModel : ObservableObject
{
    ...
    private async Task ConfirmSourceAsync()
    {
        ...
        foreach (var listed in SourceFrameList)
        {
            if (string.IsNullOrWhiteSpace(listed.id)) continue;

            if (listed.IsSelected)
        ...
        foreach (var item in selectedMap.Values)
            SelectedSourceFrames.Add(item);
    ...
    [RelayCommand]
    private void SelectAllSourceInPicker()
    {
        if (!IsSourcePickerVisible) return;
        foreach (var item in SourceFrameList)
            item.IsSelected = true;
    }

    [RelayCommand]
    private void ClearAllSourceInPicker()
    {
        if (!IsSourcePickerVisible) return;
        foreach (var item in SourceFrameList)
    ...
    private async Task LoadMoreSourceFramesAsync()
    {
        ...
        if (await ShowApiErrorAsync(r?.success, r?.message)) return;
        var merged = (r?.result ?? new()).Select(MapSource).ToList();
        if (_sourceDisplayCount >= merged.Count) return;
        foreach (var m in merged.Skip(_sourceDisplayCount).Take(PickerPageSize))
        {
            m.frameStatusDisplay = ResolveFrameStatusDisplay(m.frameStatus);
            m.IsSelected = SelectedSourceFrames.Any(x => string.Equals(x.id, m.id, StringComparison.OrdinalIgnoreCase));
    ...
    {
        if (!IsTargetPickerVisible || SelectedSourceFrames.Count == 0) return;
        var codes = SelectedSourceFrames.SelectMany(x => x.loadDetailList ?? new()).Select(x => x.materialCode ?? "").Where(x => x != "").Distinct().ToList();
        var names = SelectedSourceFrames.SelectMany(x => x.loadDetailList ?? new()).Select(x => x.materialName ?? "").Where(x => x != "").Distinct().ToList();
        await EnsureFrameStatusDictLoadedAsync();
        ...
        {
            m.frameStatusDisplay = ResolveFrameStatusDisplay(m.frameStatus);
            m.IsSelected = string.Equals(m.id, SelectedTargetFrame?.id, StringComparison.OrdinalIgnoreCase);
